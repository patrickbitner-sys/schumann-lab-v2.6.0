...
- **Carrier Pitch, Modulation Depth, Master Volume** – Fine‑tune the tone generator.
  - **Soundscapes** – Blend in natural ambience: Ocean (low‑passed noise or high‑quality loop), Rain (high‑passed noise or loop), Forest Birds (tonal chirps or loop) and Waves (slow swell pattern or loop).  When Ambient Echo is selected, only the soundscapes feed the echo network, creating a sense of being immersed in nature without any tonal carrier.
