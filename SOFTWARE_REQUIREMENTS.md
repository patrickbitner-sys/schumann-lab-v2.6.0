...
* **Ambient Echo** – Applies spatial delays to the soundscapes only.  The tonal oscillator is silenced, so you hear purely environmental sounds with echoes that follow the Schumann rhythm.  Ambient Echo inherits the same randomised pitch split as Spatial Echo and introduces a very slow modulation of the echo delay times to create a gently undulating ambience.  Ideal for a **nature‑only** meditation or for layering with the chord progression and reverb for a lush soundscape.
